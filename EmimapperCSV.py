...
def get_RMS_power(sdr):
    """Measures the RMS power with an RTL-SDR."""
...
#try to do sum of squares not welch



'''
def export_to_csv(measurements, filename='rms_power_measurements.csv'):
    """Export measurements to a CSV file."""
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Frame Number', 'RMS Power (dBm)', 'X-coordinate', 'Y-coordinate'])

        for frame_num, measurement in enumerate(measurements, start=1):
            writer.writerow([frame_num, measurement['power'], measurement['x'], measurement['y']])
'''
def visualize_power(measurements):
    frames = [i for i in range(1, len(measurements) + 1)]
    powers = [measurement['power'] for measurement in measurements]

    plt.plot(frames, powers)
...
def main():
    ...
    parser.add_argument('-c', '--camera', type=int, help='camera id (default=0)', default=1)
    parser.add_argument('-f', '--frequency', type=float, help='sets the center frequency on the SDR, in MHz (default: 300qs).', default=300)
    ...
    sdr.gain = args.gain
    sdr.set_agc_mode(0)
    # set direct sampling on Q branch
    #sdr.direct_sampling = 'Q'
    #direct sampling q branch function
    #try increase rtl gain not R82
    ...
    firstFrame = None
    firstFrameMask = None
  #  measurements = []  # To store RMS power measurements and coordinates

    # Init OpenCV object tracker objects
    tracker = cv2.TrackerCSRT_create()
    init_tracking_BB = None

    display_emi_map = False  # Flag to control displaying the EMI map
    ...
    while True:
        ...
        if init_tracking_BB is not None:
            ...
            if success:
                ...
                #measurements.append({'power': power, 'x': x + w / 2, 'y': y + h / 2})
        ...
        if key == ord("s") and init_tracking_BB is None:
            ...
            # Set the flag to display the EMI map
            display_emi_map = True

        elif key == ord("r"):
            firstFrame = None

        # If the flag is set, display the EMI map
        if display_emi_map:
